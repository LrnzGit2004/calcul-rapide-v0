...
export default function DifficultySelect() {
  const { level, operation } = useLocalSearchParams();
  ...
  const startGame = () => {
    ...
    const url = `/game?level=${encodeURIComponent(String(level))}&operation=${encodeURIComponent(String(operation))}&difficulty=${selectedDifficulty}`;
  ...
  useEffect(() => {
    ...
    Speech.speak("Choisis la difficulté", {
      language: "fr-FR",
      rate: 0.95,
      pitch: 0.8,
  ...
  useEffect(() => {
    if (modalVisible) {
      ...
      Speech.speak("Prépare toi pour les calculs.", {
        language: "fr-FR",
        pitch: 0.9,
        rate: 0.95,
